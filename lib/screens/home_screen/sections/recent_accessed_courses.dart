...
final _courses = [
  ...
  Course(
    ...
    lectures: [
      ...
      const Lecture(number: 3),
      const Lecture(number: 4),
      Lecture(number: 5, date: _now.subtract(7.h)),
    ],
  ),
];

class RecentAccessedCourses extends StatelessWidget {
  final Account user;
  ...
  Widget build(BuildContext context) {
    return CardsList(
      listHeight: 10.5.rem + 2.px, // overflows by 2.px
      indentation: AppEdgeInsets.symmetric(horizontal: inlinePadding),
      name: "Recent Accessed Courses",
      onSeeMore: () {
        //TODO: implement.
      },
      cardCount: _courses.length,
      cardBuilder: (context, index) {
        return AccessedCard(course: _courses[index], cardWidth: 14.rem);
      },
    );
  }
}
